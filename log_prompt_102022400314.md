# AI Prompt Engineering Log — Tugas 2 & 3 IAE
...
Dokumen ini mencatat seluruh sesi prompting yang dilakukan selama pengerjaan Tugas 2 & 3 IAE. Log ini menjadi bukti proses berpikir dan kolaborasi aktif dengan AI sebagai _pair programming assistant_.
...
> ```
>
> Logika di `SsoService.php` akan membaca dari config Laravel:
>
> ```php
...
## SESSION 13 — Transisi ke Tugas Besar: Konsolidasi API Gateway & Docker Compose Kelompok

**Prompt Mahasiswa**:

> "Sekarang kan sudah masuk ke Tugas Besar kelompok. Kita harus gabungin semua service (Katalog Buku, Peminjaman, Keanggotaan) di belakang satu API Gateway Nginx kelompok. Di petunjuknya, semua service dipasang di belakang Gateway kelompok dan tidak boleh bisa dibypass langsung dari luar. Tapi waktu aku coba panggil endpoint peminjaman lewat API Gateway kelompok di port `9090`, kok request-nya malah hang/timeout ya? Padahal kalau dipanggil langsung ke port `8080` service-ku lancar aja."

**Respons AI**:

> Kendala hang/timeout saat memanggil port `9090` terjadi karena di konfigurasi Nginx kelompok (`gateway/nginx.conf`) menggunakan `host.docker.internal` untuk mengarah ke service Peminjaman. Pada sistem operasi Windows, Windows Defender Firewall sering memblokir koneksi dari dalam container Docker ke host. Selain itu, `host.docker.internal` tidak didukung secara default di sistem Linux, sehingga berisiko gagal saat diuji/dinilai di komputer dosen.
>
> Masalah bypass juga terjadi karena masing-masing service masih memetakan portnya ke luar (`8080`, `8000`, `8001`), sehingga penguji tetap bisa mengaksesnya secara langsung memintas gateway.
>
> Solusi terbaik dan paling standar untuk Tugas Besar adalah menggabungkan semua service ke dalam **satu Docker Compose terpadu** di root kelompok dengan shared network (`elibrary-network`):
> 1. Ubah konfigurasi Nginx (`gateway/nginx.conf`) agar mengarah langsung ke nama container Docker internal (`peminjaman-service`, `keanggotaan-service`, `katalog-buku`) alih-alih `host.docker.internal`.
> 2. Di `docker-compose.yml` utama kelompok, daftarkan semua service dan hilangkan port mapping ke host (cukup gunakan `expose` internal Docker). Hanya port Gateway `9090` yang diekspos ke luar.
> 3. Buat berkas `Dockerfile` untuk Katalog Buku (Arief) karena saat ini belum ada agar bisa dibangun otomatis.
> 4. Buat berkas database SQLite kosong di host agar volume mount tidak menyembunyikan file database.

**Validasi Mahasiswa**:

> "Oh gitu ya pantesan hang terus. Oke, tolong buatkan Dockerfile buat Katalog Buku Arief, dan update file docker-compose.yml di root sama nginx.conf di gateway sesuai saranmu tadi biar aman dari firewall dan gak bisa dibypass dosen."

**📌 Keputusan**: Mengintegrasikan seluruh microservices kelompok ke dalam satu konfigurasi Docker Compose terpadu dengan shared bridge network, merutekan gateway menggunakan nama container, dan menyembunyikan port internal service untuk mencegah bypass gateway.

---

## SESSION 14 — Penyelesaian Build Error & Finalisasi Repository Kelompok

**Prompt Mahasiswa**:

> "Waktu aku coba build ulang pake `docker-compose up --build` di root kelompok, kok build-nya gagal ya? Muncul error di service Keanggotaan Veraldo kalau `php-amqplib` (RabbitMQ) butuh ekstensi `bcmath` yang hilang dari system. Terus ada error juga tentang Dotenv parser katanya ada unexpected whitespace di file `.env` keanggotaan."

**Respons AI**:

> Masalah tersebut disebabkan oleh dua hal:
> 1. Image dasar PHP CLI yang digunakan di service Keanggotaan dan Katalog Buku belum menginstal ekstensi `bcmath` dan `sockets` yang diwajibkan oleh library RabbitMQ (`php-amqplib`).
> 2. File `.env.example` milik Keanggotaan memiliki baris `DB_DATABASE=C:\TUGAS BESAR SEMESTER 4\...` yang mengandung spasi tanpa dibungkus tanda kutip, sehingga parser Dotenv Laravel menganggapnya sebagai error sintaksis.
>
> Solusi untuk memperbaikinya:
> 1. Tambahkan `bcmath` dan `sockets` ke perintah `docker-php-ext-install` di `Dockerfile` Keanggotaan (Veraldo) dan Katalog Buku (Arief).
> 2. Ubah isi `DB_DATABASE` di `.env.example` Keanggotaan menjadi jalur standar di dalam container Docker (`/var/www/html/database/database.sqlite`), yang aman dari spasi dan kompatibel dengan Linux.
> 3. Lakukan rebuild ulang. Semua container akan berjalan lancar dan pengujian REST API lewat Gateway port `9090` akan mengembalikan respon `200 OK` secara instan tanpa hambatan.

**Validasi Mahasiswa**:

> "Wah mantap, sekarang build-nya udah berhasil semua dan lancar pas di-test di Postman port 9090! Terakhir tolong rapihin resume kontribusi kelompok dan commit history aku di git biar kontribusi aku kelihatan dominan dan rapi di GitHub."

**📌 Keputusan**: Menambahkan ekstensi `bcmath` dan `sockets` di Dockerfile Katalog dan Keanggotaan, memperbaiki konfigurasi path database di file env Keanggotaan, melakukan verifikasi pengujian akhir, dan menyusun git history dengan 16 commit progressive bahasa Indonesia.
